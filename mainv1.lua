...
-- Create a ClickDetector for minimizedZLabel if it remains a TextLabel but needs click events reliably
...
UiTitleLabel.Font = Enum.Font.SourceSansSemibold; UiTitleLabel.Text = "ZXHELL (OPTIMIZED V2)" -- Diubah sedikit
...
elementsToToggleVisibility = {
    ...
    timerInputElements.AreaSetupDelayLabel, timerInputElements.areaSetupDelayInput, -- Added
    MinimizeButton 
}
...
local function toggleMinimize()
    ...
    if isMinimized then
        ...
        -- Calculate position to be bottom-right, considering ScreenGui.AbsoluteSize
        local screenWidth = ScreenGui.AbsoluteSize.X
        local screenHeight = ScreenGui.AbsoluteSize.Y
        local targetX = screenWidth - minimizedFrameSize.X.Offset - (0.02 * screenWidth) -- 2% margin from right
        local targetY = screenHeight - minimizedFrameSize.Y.Offset - (0.02 * screenHeight) -- 2% margin from bottom
...
-- Menggunakan ClickDetector untuk minimizedZLabel karena TextLabel tidak punya MouseButton1Click secara default
...
local function fireRemoteEnhanced(remoteName, pathType, ...)
    ...
    if pcallSuccess and pcallResult then -- Check both pcallSuccess and the explicit return from the function
        success = true 
    else 
...
local function runCycle()
    ...
    if not fireRemoteEnhanced("Reincarnate", "Base", {}) then scriptRunning = false; updateStatus("ERR_REINCARNATE"); return end
    waitSeconds(timers.reincarnate_delay); if not scriptRunning then return end

    -- >>> SECTION: PRE-COMPREHEND QI UPDATE (Requires Immortal Map + Hidden Area) <<<
    updateStatus("MAP_CHANGE_IMMORTAL_FOR_PRE_QI"); 
    if not fireRemoteEnhanced("ChangeMap", "AreaEvents", "immortal") then scriptRunning = false; updateStatus("ERR_MAP_IMMORTAL"); return end
    waitSeconds(timers.change_map_delay); if not scriptRunning then return end

    updateStatus("ENSURING_HIDDEN_AREA_FOR_PRE_QI");
    if not fireRemoteEnhanced("EnsureHiddenArea", "AreaEvents", {}) then -- Assuming "EnsureHiddenArea" remote exists
        updateStatus("ERR_HIDDEN_AREA_PRE_QI_FAIL"); 
        waitSeconds(timers.area_setup_delay); -- Wait a bit before potentially retrying cycle or halting
        return -- Skip this cycle's QI update if area setup fails
    end
    waitSeconds(timers.area_setup_delay); if not scriptRunning then return end
    updateStatus("HIDDEN_AREA_OK_FOR_PRE_QI")

    stopUpdateQi = false -- Enable Qi updates
    ...
    while scriptRunning and (tick() - preComprehendQiStartTime < timers.pre_comprehend_qi_duration) do
        if stopUpdateQi then updateStatus("PRE_COMP_QI_HALTED_EXT"); break end -- Should not happen if logic is correct
    ...
    stopUpdateQi = true -- Disable Qi updates before next phase
    if not scriptRunning then return end


    -- >>> SECTION: COMPREHEND (Requires Immortal Map + Forbidden Area) <<<
    updateStatus("MAP_CHANGE_IMMORTAL_FOR_COMP"); 
    if not fireRemoteEnhanced("ChangeMap", "AreaEvents", "immortal") then -- May be redundant if already immortal, but good for safety
        scriptRunning = false; updateStatus("ERR_MAP_IMMORTAL_COMP"); return 
    end
    waitSeconds(timers.change_map_delay); if not scriptRunning then return end

    updateStatus("ENSURING_FORBIDDEN_AREA_FOR_COMP");
    if not fireRemoteEnhanced("EnsureForbiddenArea", "AreaEvents", {}) then -- Assuming "EnsureForbiddenArea" remote exists
        updateStatus("ERR_FORBIDDEN_AREA_COMP_FAIL"); 
        waitSeconds(timers.area_setup_delay);
        return -- Skip this cycle's comprehend if area setup fails
    end
    waitSeconds(timers.area_setup_delay); if not scriptRunning then return end
    updateStatus("FORBIDDEN_AREA_OK_FOR_COMP")
    ...
    -- >>> SECTION: POST-COMPREHEND QI UPDATE (Requires Immortal Map + Hidden Area) <<<
    updateStatus("MAP_CHANGE_IMMORTAL_FOR_POST_QI"); 
    if not fireRemoteEnhanced("ChangeMap", "AreaEvents", "immortal") then 
        scriptRunning = false; updateStatus("ERR_MAP_IMMORTAL_POST_QI"); return 
    end
    waitSeconds(timers.change_map_delay); if not scriptRunning then return end

    updateStatus("ENSURING_HIDDEN_AREA_FOR_POST_QI");
    if not fireRemoteEnhanced("EnsureHiddenArea", "AreaEvents", {}) then 
        updateStatus("ERR_HIDDEN_AREA_POST_QI_FAIL"); 
        waitSeconds(timers.area_setup_delay);
        return 
    end
    waitSeconds(timers.area_setup_delay); if not scriptRunning then return end
    updateStatus("HIDDEN_AREA_OK_FOR_POST_QI")

    stopUpdateQi = false -- Enable Qi updates again
    ...
    while scriptRunning and (tick() - postComprehendQiStartTime < timers.post_comprehend_qi_duration) do
        if stopUpdateQi then updateStatus("POST_COMP_QI_HALTED_EXT"); break end
    ...
    stopUpdateQi = true -- Disable Qi updates finally for this cycle
    if not scriptRunning then return end
...
local function updateQiLoop_enhanced()
    while scriptRunning do
        -- The conditions (Immortal Map, Hidden Area) are now expected to be set by runCycle
        -- This loop just fires if stopUpdateQi is false.
        if not stopUpdateQi then 
            if not fireRemoteEnhanced("UpdateQi", "Base", {}) then
                -- Optional: Add a status update or brief pause if UpdateQi fails repeatedly
                -- updateStatus("WARN_UPDATE_QI_FAIL") 
                -- task.wait(0.5) 
            end
        end
        local interval = timers.update_qi_interval; if interval <= 0 then interval = 0.01 end
        waitSeconds(interval) -- This respects scriptRunning flag
    end
end
...
if StartButton then
    StartButton.MouseButton1Click:Connect(function()
        ...
        if scriptRunning then
            ...
            stopUpdateQi = true -- Ensure Qi is stopped until runCycle explicitly allows it
            ...
            if not mainCycleThread or coroutine.status(mainCycleThread) == "dead" then
                mainCycleThread = task.spawn(function()
                    while scriptRunning do 
                        ...
                        task.wait(1) -- Wait before restarting cycle
                    end
                    ...
                    stopUpdateQi = true -- Ensure Qi is stopped when main system halts
                end)
            end
        else 
            updateStatus("HALT_REQUESTED") 
            -- scriptRunning is now false, loops will terminate. stopUpdateQi will be set to true by the mainCycleThread when it exits.
        end
    end)
